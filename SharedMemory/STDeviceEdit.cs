...
namespace SharedMemory
{
    public class STDeviceEdit
    {
        ...
        private static VisionInfo[] m_smVSInfo;
        #endregion


        public static void InitDeviceEdit(ProductionInfo smProductionInfo, CustomOption smCustomizeInfo, VisionInfo[] smVSInfo)
        {
        ...
        public static void SaveDeviceEditLog(string strModule, string strDescription, string strOriginalValue, string strNewValue, string strLotID)
        {
            ...
            if (m_blnWantEditLog)
            {
                ...
                string m_strLotID = strLotID;
                string m_strDeviceNo = m_smProductionInfo.g_strRecipeID;
                int intVisionIndex = -1;
                string strVision = "";
                if (strModule.Length > 6)
                    strVision = strModule.Substring(0, 6);
                if (strVision == "Vision")
                {
                    if (strModule.Length > (strModule.IndexOf('>') - 6))
                        strVision = strModule.Substring(6, strModule.IndexOf('>') - 6);
                    if (int.TryParse(strVision, out intVisionIndex))
                    {
                        if (intVisionIndex != -1 && intVisionIndex < 10)
                        {
                            m_strDeviceNo = m_smProductionInfo.g_arrSingleRecipeID[intVisionIndex - 1];
                            m_strLotID = m_smProductionInfo.g_arrSingleLotID[intVisionIndex - 1];
                        }
                    }
                }
