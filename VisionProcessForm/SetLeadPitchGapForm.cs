...
{
    public partial class SetLeadPitchGapForm : Form
    {
        #region Properties
        public string ref_strToLeadNo { get { return cbo_ToLeadNo.SelectedItem.ToString(); } }
        #endregion

        public SetLeadPitchGapForm(List<int> arrLeadNo, int intCurrentFromLeadNo, string strCurrentToLeadNo)
        {
            InitializeComponent();
            ...
            for (int i = 0; i < arrLeadNo.Count; i++)
            {
                cbo_ToLeadNo.Items.Add(arrLeadNo[i].ToString());
            }
